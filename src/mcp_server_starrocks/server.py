...
# Default output format for query_and_plotly_chart when the caller does not
# pass `format`. One of: json | png | jpeg | html. Defaults to 'jpeg' (an
# inline image; no file is written). Set to 'html' to always write an
# interactive chart to STARROCKS_CHART_OUTPUT_DIR (plus an inline PNG preview).
_SUPPORTED_CHART_FORMATS = ('json', 'png', 'jpeg', 'jpg', 'html')

def _resolve_chart_default_format(value):
    v = str(value).strip().lower()
    if v not in _SUPPORTED_CHART_FORMATS:
        logger.warning(
            f"Ignoring invalid STARROCKS_CHART_DEFAULT_FORMAT={value!r}; "
            f"falling back to 'jpeg'. Valid values: {', '.join(_SUPPORTED_CHART_FORMATS)}."
        )
        return 'jpeg'
    return 'jpeg' if v == 'jpg' else v

CHART_DEFAULT_FORMAT = _resolve_chart_default_format(
    os.environ.get("STARROCKS_CHART_DEFAULT_FORMAT", "jpeg")
)
